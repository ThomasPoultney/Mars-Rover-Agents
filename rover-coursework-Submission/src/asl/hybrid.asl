										...
										if(NumActiveMiningEvents > 0) {
											...
											if(N > 0) {
												...
												.nth(0, ListOfMining, InnerList);
							      				...
							      				if(dedicatedResource(ResourceType) | dedicatedResource(null)) {
							      					!mine_resource(ID, ResourceType, Quantity, XPos, YPos);
							      					+activeMiningEvent(ID, ResourceType, Quantity, XPos, YPos);
...
-!move_rover(XDist,YDist) : true <- //.print("Failed to move, trying again");	
									 ...
									 .print("Making random move: " , RandYMovement ," , " , RandYMovement, " To try recover");
...
+!collect_resource(ID, ResourceType, Quantity, XPos, YPos) :true <- ?capacity(Cap);	     		  
        		   ...
        		   !return_to_base;	       	 		   
        		  .
-!collect_resource(ID, ResourceType, Quantity, XPos, YPos) : true <- .print("Failed to collect, removing mining events for this node");
																	  .abolish(activeMiningEvent(_,ResourceType,_,XPos,YPos)).
